...
function checkout(){if(!cart.length){document.getElementById('checkoutHint').textContent='Tambahkan produk ke keranjang dulu.';return}const nameInput=document.getElementById('customerName');const name=nameInput.value.trim();if(!name){const hint=document.getElementById('checkoutHint');hint.textContent='⚠️ Nama pelanggan wajib diisi sebelum memesan.';nameInput.focus();nameInput.setAttribute('aria-invalid','true');return}nameInput.removeAttribute('aria-invalid');const note=document.getElementById('customerNote').value.trim();const total=cart.reduce((s,x)=>s+x.price*x.qty,0);const lines=cart.map(x=>`• ${x.name} x${x.qty} = ${rupiah(x.price*x.qty)}`).join('%0A');const msg=`Halo ${C.storeName||'ShopMate AI'}, saya ingin pesan:%0A%0A${lines}%0A%0ATotal: ${rupiah(total)}%0ANama: ${encodeURIComponent(name)}${note?`%0ACatatan: ${encodeURIComponent(note)}`:''}`;const num=(C.whatsappNumber||'628XXXXXXXXXX').replace(/\D/g,'');if(num.includes('XXXXXXXX')){document.getElementById('checkoutHint').textContent='Isi nomor WhatsApp toko di config.js terlebih dahulu.';return}window.open(`https://wa.me/${num}?text=${msg}`,'_blank')}
function tanyaShopper(){const num=(C.whatsappNumber||'628XXXXXXXXXX').replace(/\D/g,'');if(num.includes('XXXXXXXX')){alert('Isi nomor WhatsApp toko di config.js terlebih dahulu.');return}const msg='Halo Toko Keripik Pade, saya mau daftar menjadi Reseller Pade.';window.open(`https://wa.me/${num}?text=${encodeURIComponent(msg)}`,'_blank')}
